...
export default function GameplayScreen({ charData, onRestart }) {
  ...
  const checkResume = useCallback(async () => {
    ...
    const save = await loadSave(authUser?.uid, charData.cont, charData.job);
    if (!save) return;
    if (save.character?.cont !== charData.cont || save.character?.job !== charData.job) return;
    const savedTurns = save.turns ?? save.history?.length ?? 0;
    if (savedTurns > historyRef.current.length) {
      const turns = save.turns ?? save.history?.length ?? 0;
      const chap = save.chapter && save.chapter !== '— 대기 중 —' ? save.chapter : '';
      const info = chap ? `${chap} (${turns}턴)` : `${turns}턴 진행됨`;
      setResumeBanner({ save, info });
    }
  }, [authUser, charData.cont, charData.job]);

  // ── 게임 시작 ────────────────────────────────────────────────────────
  const startGame = useCallback(async () => {
    const save = await loadSave(authUser?.uid, charData.cont, charData.job);
    if (save && save.character?.cont === charData.cont && save.character?.job === charData.job
        && (save.turns ?? save.history?.length ?? 0) > 0) {
      lastFocusRef.current = Date.now();
      const turns = save.turns ?? save.history?.length ?? 0;
      const chap = save.chapter && save.chapter !== '— 대기 중 —' ? save.chapter : '';
      const info = chap ? `${chap} (${turns}턴)` : `${turns}턴 진행됨`;
      setResumeBanner({ save, info, isStart: true });
      return;
    }
    callGM(`게임을 시작합니다. 테라 노바의 첫 장면을 시작해주세요. ${jobName}(${contName} 출신)으로서 테라 노바에 막 도착한 상황으로 시작해 주세요.`);
  }, [authUser, charData.cont, charData.job, callGM, contName, jobName]);

  // ── 초기화 & 이벤트 리스너 ──────────────────────────────────────────
  useEffect(() => {
    updateChapterHUD(charRef.current);
    if (apiKey && !startedRef.current) {
      startedRef.current = true;
      startGame();
    }
    const onVis = () => { if (document.visibilityState==='visible') checkResume(); };
  ...
  return (
    <div className="gp-root">
      ...
      {/* 이어하기 배너 */}
      {resumeBanner && (
        <div className="resume-banner show">
          <span className="resume-msg">저장된 진행 상황이 있습니다 — {resumeBanner.info}</span>
          <div className="resume-btns">
            <button className="resume-yes" onClick={() => { applyResume(resumeBanner.save); setResumeBanner(null); }}>이어하기</button>
            <button className="resume-no" onClick={() => {
              setResumeBanner(null);
              if (resumeBanner.isStart) {
                callGM(`게임을 시작합니다. 테라 노바의 첫 장면을 시작해주세요. ${jobName}(${contName} 출신)으로서 테라 노바에 막 도착한 상황으로 시작해 주세요.`);
              }
            }}>무시</button>
          </div>
