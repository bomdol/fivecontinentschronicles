...
export const ROLE_GROWTH = {
  warrior: { baseHp:120, hpPerLv:90, baseMp:30,  mpPerLv:15, mpStat:'체력' },
  mage:    { baseHp:60,  hpPerLv:40, baseMp:100, mpPerLv:65, mpStat:'지력' },
  rogue:   { baseHp:80,  hpPerLv:55, baseMp:50,  mpPerLv:30, mpStat:'민첩' },
  support: { baseHp:85,  hpPerLv:55, baseMp:90,  mpPerLv:55, mpStat:'지혜' },
  tech:    { baseHp:90,  hpPerLv:60, baseMp:60,  mpPerLv:40, mpStat:'지력' },
  special: { baseHp:75,  hpPerLv:50, baseMp:80,  mpPerLv:60, mpStat:'지혜' },
};
...
// ─── 직업별 기본 방어 등급(AC) ────────────────────────────────────────
// base: 기본 AC / addDex: 민첩 수정치 추가 여부
export const JOB_BASE_AC = {
  kn: { base:17, addDex:false }, // 마법 기사 — 마법 중갑
  am: { base:13, addDex:true  }, // 대마법사 — 마법 로브
  ma: { base:14, addDex:true  }, // 마법 암살자 — 경갑
  hl: { base:14, addDex:true  }, // 성약사 — 경판금
  fb: { base:12, addDex:true  }, // 금서 사용자 — 무갑
  ar: { base:18, addDex:false }, // 기갑병 — 동력 갑옷
  iv: { base:14, addDex:true  }, // 발명가 — 개조 갑옷
  hk: { base:13, addDex:true  }, // 해커 — 경장비
  md: { base:13, addDex:true  }, // 의무병 — 전투 의료복
  nw: { base:16, addDex:false }, // 새사람 — 사이버 신체
  sw: { base:15, addDex:true  }, // 검객 — 무협 경장
  sc: { base:13, addDex:true  }, // 술사 — 도사복
  jk: { base:15, addDex:true  }, // 자객 — 암살자 복장
  us: { base:14, addDex:true  }, // 의선 — 도복
  dh: { base:15, addDex:true  }, // 달마 내공자 — 금강마체
  fc: { base:16, addDex:false }, // 역장사 — 역장 방어막
  tk: { base:13, addDex:true  }, // 염동력자 — 염동 방어
  jy: { base:12, addDex:true  }, // 잠영술사 — 무갑
  ep: { base:12, addDex:true  }, // 초능력 의사 — 무갑
  aw: { base:13, addDex:true  }, // 각성자 — 불안정한 에너지 방어
};

// ─── 직업별 평타 (기본 공격 — MP/HP 비용 없음) ───────────────────────
// 전투 중 항상 사용 가능. damage:{dice,sides,bonus_stat} + attackStat 사용.
export const JOB_BASIC_ATTACK = {
  // 마법 대륙
  kn: { name:'기본 베기',   effect:'검으로 기본 베기',           cost:{}, damage:{dice:1,sides:8,bonus_stat:'근력'}, attackStat:'근력' },
  am: { name:'마력 볼트',   effect:'기본 마력 투사체 발사',       cost:{}, damage:{dice:1,sides:4,bonus_stat:'지력'}, attackStat:'지력' },
  ma: { name:'빠른 베기',   effect:'단검으로 빠르게 벤다',        cost:{}, damage:{dice:1,sides:6,bonus_stat:'민첩'}, attackStat:'민첩' },
  hl: { name:'기본 공격',   effect:'무기로 기본 타격',            cost:{}, damage:{dice:1,sides:4,bonus_stat:'근력'}, attackStat:'근력' },
  fb: { name:'저주 화살',   effect:'기초 저주력을 담아 발사',      cost:{}, damage:{dice:1,sides:4,bonus_stat:'지력'}, attackStat:'지력' },
  // 과학 대륙
  ar: { name:'동력 주먹',   effect:'동력 갑옷으로 강타',          cost:{}, damage:{dice:1,sides:8,bonus_stat:'근력'}, attackStat:'근력' },
  iv: { name:'충격 도구',   effect:'발명품으로 기본 공격',         cost:{}, damage:{dice:1,sides:6,bonus_stat:'지력'}, attackStat:'지력' },
  hk: { name:'전기 충격',   effect:'장비로 전기 충격 발사',        cost:{}, damage:{dice:1,sides:4,bonus_stat:'지력'}, attackStat:'지력' },
  md: { name:'긴급 타격',   effect:'의료 도구로 기본 타격',        cost:{}, damage:{dice:1,sides:4,bonus_stat:'근력'}, attackStat:'근력' },
  nw: { name:'기계 주먹',   effect:'사이버 신체로 강타',           cost:{}, damage:{dice:1,sides:6,bonus_stat:'체력'}, attackStat:'체력' },
  // 무협 대륙
  sw: { name:'기본 베기',   effect:'검으로 기본 베기',            cost:{}, damage:{dice:1,sides:8,bonus_stat:'근력'}, attackStat:'근력' },
  sc: { name:'기초 술법',   effect:'기초 원소 공격',              cost:{}, damage:{dice:1,sides:4,bonus_stat:'지혜'}, attackStat:'지혜' },
  jk: { name:'단검 찌르기', effect:'단검으로 급소를 찌른다',       cost:{}, damage:{dice:1,sides:6,bonus_stat:'민첩'}, attackStat:'민첩' },
  us: { name:'격타',        effect:'기본 격투 타격',              cost:{}, damage:{dice:1,sides:4,bonus_stat:'근력'}, attackStat:'근력' },
  dh: { name:'기본 격파',   effect:'내공을 실은 기본 타격',        cost:{}, damage:{dice:1,sides:6,bonus_stat:'근력'}, attackStat:'근력' },
  // 초능력 대륙
  fc: { name:'역장 충격파', effect:'기본 역장 에너지 방출',        cost:{}, damage:{dice:1,sides:6,bonus_stat:'지혜'}, attackStat:'지혜' },
  tk: { name:'염동 투사',   effect:'주변 물체를 염동력으로 투척',  cost:{}, damage:{dice:1,sides:6,bonus_stat:'민첩'}, attackStat:'민첩' },
  jy: { name:'기습 공격',   effect:'은신에서 기본 공격',           cost:{}, damage:{dice:1,sides:4,bonus_stat:'민첩'}, attackStat:'민첩' },
  ep: { name:'정신 충격',   effect:'정신력으로 기본 공격',         cost:{}, damage:{dice:1,sides:4,bonus_stat:'지혜'}, attackStat:'지혜' },
  aw: { name:'에너지 방출', effect:'각성 에너지를 기본 방출',      cost:{}, damage:{dice:1,sides:6,bonus_stat:'지혜'}, attackStat:'지혜' },
};

// ─── XP 레벨 임계값 (0=Lv1, 1=Lv2, ..., 11=Lv12) ────────────────────
export const XP_LEVELS = [0, 300, 900, 2700, 6500, 14000, 23000, 34000, 48000, 64000, 85000, 100000];

// ─── 몬스터 레벨별 기본 경험치 ───────────────────────────────────────
export const XP_BY_MONSTER_LEVEL = {
...
// ─── 직업별 스킬 ──────────────────────────────────────────────────────
// passive:false 스킬 중 damage 필드가 있으면 엔진이 공격 판정을 처리.
// damage:{dice,sides,bonus_stat} — 주사위 N개×S면 + 능력치 수정치
// attackStat: 공격 판정에 사용하는 능력치명 (한국어)
export const JOB_SKILLS = {
  ...
  kn: [ // 마법 기사 (warrior)
    ...
    { lv:1,  name:'방패 강타',    passive:false, cost:{mp:10}, effect:'방패에 마력을 실어 공격. 피해+30%, 적 1명 넘어짐(STR 판정 DC12)',
      attackStat:'근력', damage:{dice:1,sides:8,bonus_stat:'근력'} },
    { lv:3,  name:'신성 방어',    passive:false, cost:{mp:20}, effect:'다음 공격 피해 50% 감소, 1턴 지속' },
    { lv:5,  name:'성기사의 빛',  passive:false, cost:{mp:40}, effect:'신성한 빛으로 아군 전체 HP 60 회복', healAmount:60 },
    ...
    { lv:12, name:'성전사의 심판',passive:false, cost:{mp:150},effect:'신성 에너지 폭발, 적 전체 HP 250 피해 + 속박 1턴',
      attackStat:'근력', damage:{dice:3,sides:8,bonus_stat:'근력'} },
  ],
  ...
  am: [ // 대마법사 (mage)
    ...
    { lv:1,  name:'마법 화살',   passive:false, cost:{mp:10}, effect:'순수 마력 화살 발사, 단일 대상 INT 기반 피해',
      attackStat:'지력', damage:{dice:1,sides:6,bonus_stat:'지력'} },
    { lv:3,  name:'불꽃 폭발',   passive:false, cost:{mp:30}, effect:'화염 구체 폭발, 적 전체 화염 피해',
      attackStat:'지력', damage:{dice:2,sides:6,bonus_stat:'지력'} },
    ...
    { lv:7,  name:'고대의 분노', passive:false, cost:{mp:80}, effect:'고대 마법 방출, 단일 대상에게 INT×4 피해',
      attackStat:'지력', damage:{dice:4,sides:6,bonus_stat:'지력'} },
    ...
    { lv:12, name:'소멸의 섬광', passive:false, cost:{mp:200},effect:'적 전체에 소멸 마법 폭격, INT×6 피해 + 3턴간 마력 봉인',
      attackStat:'지력', damage:{dice:6,sides:6,bonus_stat:'지력'} },
  ],
  ...
  ma: [ // 마법 암살자 (rogue)
    ...
    { lv:1,  name:'마법 단검',   passive:false, cost:{mp:10}, effect:'마력 주입 단검 투척, 단일 대상 DEX+INT 피해',
      attackStat:'민첩', damage:{dice:1,sides:6,bonus_stat:'민첩'} },
    ...
    { lv:5,  name:'마비 독',     passive:false, cost:{mp:40}, effect:'독 주입, 적 1명 3턴간 매 턴 HP -20 + CON 판정(DC13) 실패 시 1턴 행동 불능',
      attackStat:'민첩', damage:{dice:1,sides:4,bonus_stat:'민첩'} },
    ...
    { lv:9,  name:'심장 관통',   passive:false, cost:{mp:90}, effect:'급소를 노린 일격, 단일 대상에게 DEX×5 피해 + 출혈 3턴',
      attackStat:'민첩', damage:{dice:5,sides:6,bonus_stat:'민첩'} },
    { lv:12, name:'죽음의 춤',   passive:false, cost:{mp:150},effect:'3턴간 매 턴 그림자 속에서 기습, 공격마다 DEX×3 피해 + 기절',
      attackStat:'민첩', damage:{dice:3,sides:6,bonus_stat:'민첩'} },
  ],

  hl: [ // 성약사 (support)
    { lv:1,  name:'치료의 손길', passive:true,  cost:{},      effect:'매 턴 시작 시 아군 1명 HP 5 자동 회복. WIS 판정에 이점' },
    { lv:1,  name:'치유술',      passive:false, cost:{mp:15}, effect:'빛 마법으로 아군 1명 HP 50 + WIS 수정치×5 회복', healAmount:50 },
    { lv:3,  name:'정화',        passive:false, cost:{mp:20}, effect:'마법적 오염 제거, 아군 1명 상태이상 모두 해제' },
    { lv:5,  name:'부활의 빛',   passive:false, cost:{mp:50}, effect:'강력한 치유 마법, 아군 1명 HP 150 회복', healAmount:150 },
    { lv:7,  name:'신성 방어막', passive:false, cost:{mp:80}, effect:'아군 전체 1턴간 피해 무효화 + HP 50 회복', healAmount:50 },
    { lv:9,  name:'기적',        passive:false, cost:{mp:150},effect:'신의 가호, 아군 전체 HP 완전 회복 + 모든 상태이상 해제', healAmount:9999 },
    { lv:12, name:'성전의 가호', passive:false, cost:{mp:220},effect:'3턴간 아군 전체 피해 50% 감소 + 매 턴 HP 80 회복', healAmount:80, buffEffect:{hpRegen:80, turns:2} },
  ],
  ...
  ar: [ // 기갑병 (warrior)
    ...
    { lv:1,  name:'기계 주먹',   passive:false, cost:{mp:10}, effect:'동력 강화 주먹 공격, STR×3 피해 + 넘어짐(STR 판정 DC12)',
      attackStat:'근력', damage:{dice:2,sides:6,bonus_stat:'근력'} },
    ...
    { lv:5,  name:'로켓 돌격',   passive:false, cost:{mp:40}, effect:'추진기 발동 전속 돌격, 적 전체에 충격 + 1턴 행동 불능(CON 판정 DC13)',
      attackStat:'근력', damage:{dice:2,sides:8,bonus_stat:'근력'} },
    { lv:7,  name:'EMP 충격파',  passive:false, cost:{mp:70}, effect:'전자기 펄스 방출, 적 전체 HP 120 피해 + 기계형 적 2턴 기절',
      attackStat:'근력', damage:{dice:3,sides:6,bonus_stat:'근력'} },
    { lv:9,  name:'자기 수리',   passive:false, cost:{mp:80}, effect:'긴급 수리 프로토콜 작동, HP 250 회복', healAmount:250 },
  ...
  iv: [ // 발명가 (tech)
    ...
    { lv:5,  name:'전기 충격망', passive:false, cost:{mp:50}, effect:'전기 충격 장치 기동, 적 전체 HP 100 피해 + 턴 지연 1(DEX 판정 DC13)',
      attackStat:'지력', damage:{dice:2,sides:8,bonus_stat:'지력'} },
    ...
    { lv:9,  name:'핵 배터리 폭주',passive:false,cost:{hp:50,mp:80},effect:'배터리 과부하 유도 폭발, 적 전체에 INT×5 피해. 자신도 HP 50 소모',
      attackStat:'지력', damage:{dice:5,sides:6,bonus_stat:'지력'} },
  ...
  md: [ // 의무병 (support)
    ...
    { lv:1,  name:'진통제 투여', passive:false, cost:{mp:15}, effect:'응급 진통제 처치, 아군 1명 HP 50 회복 + 고통·출혈 해제', healAmount:50 },
    { lv:3,  name:'독소 분석',   passive:false, cost:{mp:20}, effect:'독·방사능·화학 상태이상 진단 및 제거, 아군 1명 적용' },
    { lv:5,  name:'현장 수술',   passive:false, cost:{mp:50}, effect:'1턴 소요 집중 수술, 아군 1명 HP 180 회복 + 부상 상태이상 제거', healAmount:180 },
    { lv:7,  name:'강화 약물',   passive:false, cost:{mp:60}, effect:'전투 강화제 투여, 아군 1명 2턴간 ATK+50% + 피해 받을 때 HP 자동 20 회복' },
    { lv:9,  name:'소생술',      passive:false, cost:{mp:130},effect:'임상사망 상태 아군 1명을 HP 100으로 소생', healAmount:100 },
    { lv:12, name:'완전 치료',   passive:false, cost:{mp:200},effect:'아군 전체 HP 완전 회복 + 모든 상태이상 제거 + 1턴간 피해 면역', healAmount:9999 },
  ],
  ...
  nw: [ // 새사람 (special)
    ...
    { lv:5,  name:'과부하 방출', passive:false, cost:{mp:50}, effect:'체내 에너지 과부하 방출, 적 전체 전기 피해 + 1턴 기절(CON 판정 DC14)',
      attackStat:'체력', damage:{dice:2,sides:6,bonus_stat:'체력'} },
    { lv:7,  name:'나노봇 수리', passive:false, cost:{mp:60}, effect:'체내 나노봇 활성화, HP 200 회복 + 2턴간 매 턴 HP 30 회복', healAmount:200, buffEffect:{hpRegen:30, turns:2} },
  ...
  sw: [ // 검객 (warrior)
    ...
    { lv:3,  name:'선풍검무',    passive:false, cost:{mp:30}, effect:'회전 검무로 적 전체 베기, STR×2 피해',
      attackStat:'근력', damage:{dice:2,sides:4,bonus_stat:'근력'} },
    ...
    { lv:9,  name:'만초귀종',    passive:false, cost:{mp:100},effect:'신속 연속 10회 공격, 각 공격마다 STR×2 피해',
      attackStat:'근력', damage:{dice:4,sides:6,bonus_stat:'근력'} },
    { lv:12, name:'천하제일검',  passive:false, cost:{mp:200},effect:'절세 무공의 극의. 적 전체에 STR×8 피해 + 2턴 기절, 저항 불가',
      attackStat:'근력', damage:{dice:6,sides:8,bonus_stat:'근력'} },
  ],
  ...
  sc: [ // 술사 (mage)
    ...
    { lv:1,  name:'기폭',        passive:false, cost:{mp:10}, effect:'기 폭발 투사, 단일 대상 WIS 기반 피해',
      attackStat:'지혜', damage:{dice:1,sides:6,bonus_stat:'지혜'} },
    { lv:3,  name:'화기 분출',   passive:false, cost:{mp:30}, effect:'화(火)의 기 폭발, 적 전체 WIS×3 화염 피해',
      attackStat:'지혜', damage:{dice:2,sides:6,bonus_stat:'지혜'} },
  ...
  jk: [ // 자객 (rogue)
    ...
    { lv:1,  name:'암기 투척',   passive:false, cost:{mp:10}, effect:'숨겨둔 암기 투척, 단일 대상 DEX×3 피해',
      attackStat:'민첩', damage:{dice:2,sides:6,bonus_stat:'민첩'} },
    ...
    { lv:9,  name:'십자 암격',   passive:false, cost:{mp:90}, effect:'여러 급소 동시 타격, 단일 대상 DEX×6 피해 + 기절 1턴',
      attackStat:'민첩', damage:{dice:4,sides:6,bonus_stat:'민첩'} },
  ...
  us: [ // 의선 (support)
    ...
    { lv:1,  name:'기공 치료',   passive:false, cost:{mp:15}, effect:'내공으로 경혈 자극, 아군 1명 HP 50 회복', healAmount:50 },
    { lv:3,  name:'해독 침술',   passive:false, cost:{mp:20}, effect:'독·마비·출혈 등 상태이상 제거, 아군 1명' },
    { lv:5,  name:'영약 조제',   passive:false, cost:{mp:50}, effect:'영약 즉석 조제, 아군 1명 HP 160 회복 + 2턴간 STR+WIS 판정 이점', healAmount:160 },
    { lv:7,  name:'봉혈',        passive:false, cost:{mp:60}, effect:'적 급소 경혈 봉쇄, 적 1명 2턴간 스킬 사용 불가(CON 판정 DC15로 저항)',
      saveCheck:{stat:'CON',dc:15}, onFailEffect:{type:'silenced',turns:2,target:'single'} },
    { lv:9,  name:'대환단',      passive:false, cost:{mp:130},effect:'전설적 영약 투여, 아군 전체 HP 200 회복 + 상태이상 제거', healAmount:200 },
    { lv:12, name:'불로장생공',  passive:false, cost:{mp:220},effect:'불로장생 내공 전수, 3턴간 아군 전체 HP 자동 회복(매 턴 HP 100) + 사망 무효 1회', healAmount:100, buffEffect:{hpRegen:100, turns:2} },
  ],
  ...
  dh: [ // 달마 내공자 (special)
    ...
    { lv:3,  name:'주화입마 유도',passive:false,cost:{mp:25}, effect:'적에게 주화입마 유발, WIS 판정(DC14) 실패 시 자신 공격 1턴',
      saveCheck:{stat:'WIS',dc:14}, onFailEffect:{type:'confused',turns:1,target:'single'} },
    { lv:5,  name:'금강마체',    passive:false, cost:{mp:50}, effect:'마기로 신체 강화, 2턴간 피해 70% 감소 + 매 턴 HP 30 회복', healAmount:30, buffEffect:{hpRegen:30, turns:1} },
  ...
  tk: [ // 염동력자 (tech)
    ...
    { lv:3,  name:'다중 투사',   passive:false, cost:{mp:25}, effect:'여러 물체를 동시 투사, 적 전체에 DEX×2 피해',
      attackStat:'민첩', damage:{dice:2,sides:4,bonus_stat:'민첩'} },
  ...
  jy: [ // 잠영술사 (rogue)
    ...
    { lv:1,  name:'정신 교란',   passive:false, cost:{mp:10}, effect:'단기 정신 혼란 유도, 적 1명 WIS 판정(DC12) 실패 시 1턴 혼란',
      saveCheck:{stat:'WIS',dc:12}, onFailEffect:{type:'confused',turns:1,target:'single'} },
    { lv:3,  name:'환각',        passive:false, cost:{mp:25}, effect:'강렬한 환각 주입, 적 1명 2턴간 혼란(WIS 판정 DC14)',
      saveCheck:{stat:'WIS',dc:14}, onFailEffect:{type:'confused',turns:2,target:'single'} },
    { lv:5,  name:'공포 심기',   passive:false, cost:{mp:40}, effect:'심층 공포 유도, 적 1명 3턴간 아군 공격 불가(WIS 판정 DC15 저항)',
      saveCheck:{stat:'WIS',dc:15}, onFailEffect:{type:'feared',turns:3,target:'single'} },
    { lv:7,  name:'정신 마비',   passive:false, cost:{mp:70}, effect:'정신 회로 차단, 적 1명 1턴 완전 기절(WIS 판정 DC16, 저항 불가 시 2턴)',
      saveCheck:{stat:'WIS',dc:16}, onFailEffect:{type:'stunned',turns:1,target:'single'} },
    { lv:9,  name:'집단 환각',   passive:false, cost:{mp:100},effect:'적 전체 강렬한 환각, 2턴간 혼란 + 서로 공격(WIS 판정 DC15)',
      saveCheck:{stat:'WIS',dc:15}, onFailEffect:{type:'confused',turns:2,target:'all'} },
    { lv:12, name:'정신 소각',   passive:false, cost:{mp:160},effect:'적 1명의 정신을 완전 소각, 즉시 기절 3턴 + 전투 후에도 기억·스킬 일시 봉인',
      attackStat:'지혜', damage:{dice:4,sides:8,bonus_stat:'지혜'} },
  ],

  ep: [ // 초능력 의사 (support)
    { lv:1,  name:'고통 흡수',   passive:true,  cost:{},      effect:'아군의 고통을 자동 감지·흡수. 아군 디버프 지속시간 1턴 감소. WIS 판정에 이점' },
    { lv:1,  name:'감응 치유',   passive:false, cost:{mp:15}, effect:'초능력으로 상처 직접 치유, 아군 1명 HP 50 회복', healAmount:50 },
    { lv:3,  name:'신경 회복',   passive:false, cost:{mp:20}, effect:'신경계 복원, 마비·기절·혼란 등 상태이상 제거 (아군 1명)' },
    { lv:5,  name:'생명력 전이', passive:false, cost:{hp:50}, effect:'자신 HP 50 소모, 아군 1명 HP 120 회복 (MP 소모 없음)', healAmount:120 },
    { lv:7,  name:'집단 감응',   passive:false, cost:{mp:80}, effect:'아군 전체 심리 안정화, HP 80 회복 + 모든 정신계 상태이상 해제', healAmount:80 },
    { lv:9,  name:'소생 감응',   passive:false, cost:{mp:130},effect:'임상사망 아군 1명 HP 80으로 소생 + 2턴간 고통 면역', healAmount:80 },
    { lv:12, name:'공명',        passive:false, cost:{mp:200},effect:'전장 전체 생명 에너지 공명. 아군 전체 HP 완전 회복 + 3턴간 피해 받을 때마다 자동 HP 50 회복', healAmount:9999, buffEffect:{hpRegen:50, turns:3} },
  ],
...
// 능력치 문자열에서 특정 스탯 수정치 추출 (예: "근력 15, 민첩 12, ...")
export function getStatMod(statsStr, statName) {
  const m = statsStr.match(new RegExp(statName + '\\s+(\\d+)'));
  if (!m) return 0;
  return Math.floor((parseInt(m[1]) - 10) / 2);
}

// 직업 AC 계산 (기본 AC + 민첩 수정치)
export function getPlayerAC(jobId, statsStr) {
  const info = JOB_BASE_AC[jobId] ?? { base: 12, addDex: true };
  const dexMod = info.addDex ? getStatMod(statsStr, '민첩') : 0;
  return info.base + dexMod;
}

// 레벨·직업·능력치로 최대 HP 계산
export function calcMaxHp(jobId, level, statsStr) {
  const role = JOB_ROLE[jobId] || 'rogue';
  const g = ROLE_GROWTH[role];
  const conMod = getStatMod(statsStr, '체력');
  return g.baseHp + (level - 1) * g.hpPerLv + conMod * level;
}

// 레벨·직업·능력치로 최대 MP 계산
export function calcMaxMp(jobId, level, statsStr) {
  const role = JOB_ROLE[jobId] || 'rogue';
  const g = ROLE_GROWTH[role];
...
// 몬스터 처치 경험치 계산
export function getMonsterXp(monsterLevel, playerLevel) {
...
export function getAvailableSkills(jobId, level) {
  return (JOB_SKILLS[jobId] || []).filter(s => s.lv <= level);
}

// ─── 시스템 프롬프트용 텍스트 생성 ──────────────────────────────────

export function buildJobSkills(jobId, level) {
